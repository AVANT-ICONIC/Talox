...
test.describe("Scenario 6b — Grok agent-to-agent (free mode)", () => {
	...
	test("Step 3 — sends a message to Grok as a free user", async () => {
		...
		const skipLogin =
			(await talox.findElement("Continue without signing in", "button")) ??
		...
		if (!typed) {
			const inputEl =
				(await talox.findElement("Message Grok", "textbox")) ??
		...
		try {
			const sendEl =
				(await talox.findElement("Send message", "button")) ??
