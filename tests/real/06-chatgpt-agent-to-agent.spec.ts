...
test.describe("Scenario 6 — ChatGPT agent-to-agent (guest mode)", () => {
	...
	test.beforeAll(async () => {
		...
		await talox.launch("chatgpt-agent", "sandbox", "chromium");
	});
	...
	test("Step 3 — sends a message to ChatGPT as a guest", async () => {
		...
		const stayLoggedOut =
			(await talox.findElement("Stay logged out", "button")) ??
		...
		let sent = false;
		try {
			const sendEl = (await talox.findElement("Send message", "button")) ?? (await talox.findElement("Send", "button"));
			if (sendEl) {
