...
vi.mock('../../src/types/index.js', async (importOriginal) => {
  ...
  return {
    ...
    diffPageState: vi.fn().mockReturnValue({
      fromUrl: '', toUrl: '', urlChanged: false,
      fromTitle: '', toTitle: '', titleChanged: false,
      nodesAdded: [], nodesRemoved: [], nodesModified: [],
      bugsAdded: [], bugsRemoved: [],
      interactiveElementsAdded: [], interactiveElementsRemoved: [],
    }),
  };
});
...
function createMockElement(box: { x: number; y: number; width: number; height: number } | null = { x: 100, y: 200, width: 80, height: 40 }) {
  return {
...
describe('ActionExecutor', () => {
  ...
  function makeExecutor(overrides: Record<string, any> = {}) {
    ...
    const executor = new ActionExecutor(
      settings, events, artifactBuilder, policyEngine, semanticMapper,
      deps.getPage, deps.getActiveStateCollector, deps.getProfile,
      deps.getCurrentLastMousePos, deps.setCurrentLastMousePos,
      deps.getAttentionFrame, deps.clampToFrame, deps.findElementInFrame,
      deps.riskyActionHook, deps.recordActivity, deps.getCursorStepCallback,
    );

    return { executor, mockPage, mockCollector, settings, events, artifactBuilder, policyEngine, semanticMapper, deps, pageRef };
  }
  ...
  describe('navigate', () => {
    ...
    it('throws policy violation when URL is not allowed', async () => {
      ...
      await expect(
        executor.navigate('https://blocked.com', false, vi.fn(), null, rulesEngine)
    ...
    it('emits navigation event with state url and title', async () => {
      ...
      expect(events.emit).toHaveBeenCalledWith('navigation', expect.objectContaining({
        url: 'https://example.com',
        title: 'Test Page',
      }));
    });
  ...
  describe('click', () => {
    ...
    it('uses HumanMouse.click in biomechanical mode (humanStealth > 0)', async () => {
      ...
      expect(events.emit).toHaveBeenCalledWith('cursorClicked', expect.objectContaining({
        x: expect.any(Number),
        y: expect.any(Number),
      }));
    });
    ...
    it('records an artifact for the click action', async () => {
      ...
      expect(artifactBuilder.addAction).toHaveBeenCalledWith('click', expect.objectContaining({
        selector: '',
      }));
    });
  ...
  describe('type', () => {
    ...
    it('records an artifact for the type action', async () => {
      ...
      // The reliability pre-flight resolves the selector; mock returns ''.
      expect(artifactBuilder.addAction).toHaveBeenCalledWith('type', expect.objectContaining({
        selector: '',
        text: 'hello',
      }));
    });
  ...
  describe('findElement', () => {
    ...
    it('finds element by label match and returns selector + boundingBox', async () => {
      const { executor, semanticMapper } = makeExecutor();
      const elements = [
        { id: '#btn1', label: 'Submit Form', name: 'Submit', confidence: 0.9, boundingBox: { x: 10, y: 20, width: 80, height: 30 } },
        { id: '#btn2', label: 'Cancel', name: 'Cancel', confidence: 0.7, boundingBox: { x: 100, y: 20, width: 80, height: 30 } },
      ];
