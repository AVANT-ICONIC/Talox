...
describe('ChallengeResolver', () => {
  ...
  describe('suggestStrategy', () => {
    ...
    it('consent-wall → auto-click-accept', () => expect(resolver.suggestStrategy('consent-wall')).toBe('auto-click-accept'));
    ...
    it('empty-shell-spa → wait-hydration', () => expect(resolver.suggestStrategy('empty-shell-spa')).toBe('wait-hydration'));
  ...
  describe('rate-limited — backoff-retry', () => {
    it('resolves after successful reload', async () => {
      let reloads = 0;
      const page = {
        reload: vi.fn().mockImplementation(async () => { reloads++; }),
  ...
  describe('empty-shell-spa — wait-hydration', () => {
    it('resolves when interactive elements appear', async () => {
      ...
      const r = await new ChallengeResolver({ maxRetries: 5, baseDelayMs: 1, maxDelayMs: 5 }).resolve(
        challenge('empty-shell-spa'), page,
      );
      expect(r.resolved).toBe(true);
    });
    ...
    it('fails after timeout if no elements appear', async () => {
      ...
      const r = await new ChallengeResolver({ maxRetries: 2, baseDelayMs: 1, maxDelayMs: 5, spaHydrationTimeoutMs: 50 }).resolve(
        challenge('empty-shell-spa'), page,
      );
      expect(r.resolved).toBe(false);
