...
import { PerceptionStack, PERCEPTION_PRESETS } from '../../src/core/PerceptionStack';
...
function makeBaseState(overrides: Partial<TaloxPageState> = {}): TaloxPageState {
  return {
    ...
    nodes: [
      { id: 'n1', role: 'button', name: 'Submit', boundingBox: { x: 0, y: 0, width: 100, height: 40 } },
    ],
    interactiveElements: [],
    bugs: [],
    ...overrides,
...
describe('PerceptionStack', () => {
  ...
  describe('cheap preset', () => {
    ...
    it('strips bugs', async () => {
      const state = makeBaseState({ bugs: [{ type: 'missing-label', message: 'x', severity: 'warning', nodeId: 'n1' }] });
      const stack = new PerceptionStack(makeCollector(state));
      const result = await stack.collect('cheap');
  ...
  describe('heavy preset', () => {
    ...
    it('runs rules engine when provided', async () => {
      const engine = makeRulesEngine([{ type: 'layout-bug', message: 'overlap detected', severity: 'error', nodeId: 'n1' }]);
      const stack = new PerceptionStack(makeCollector(makeBaseState()));
