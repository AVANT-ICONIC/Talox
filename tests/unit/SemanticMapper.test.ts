...
import { createSemanticMapper, SemanticMapper } from "../../src/core/SemanticMapper";
...
describe("SemanticMapper", () => {
	...
	describe("knowledge profiles", () => {
		...
		it("addKnowledgeProfile adds a custom profile", () => {
			const profile: KnowledgeProfile = {
				...
				version: "1.0.0",
				mappings: [{ selector: "#search", semanticType: "search", label: "custom-search", priority: 10 }],
			};
	...
	describe("heuristic role mapping", () => {
		it.each([
			...
			["article", "article"],
			["form", "form"],
		] as [string, SemanticEntityType][])('maps role "%s" to type "%s"', (role, expectedType) => {
	...
	describe("filtering and sorting", () => {
		const entities = () => [
			mapper.mapNode(
				makeNode({ id: "n1", role: "button", name: "A", boundingBox: { x: 0, y: 100, width: 50, height: 20 } }),
			),
			mapper.mapNode(
				makeNode({ id: "n2", role: "link", name: "B", boundingBox: { x: 0, y: 50, width: 50, height: 20 } }),
			),
			mapper.mapNode(
				makeNode({ id: "n3", role: "heading", name: "C", boundingBox: { x: 0, y: 0, width: 50, height: 20 } }),
			),
		];
