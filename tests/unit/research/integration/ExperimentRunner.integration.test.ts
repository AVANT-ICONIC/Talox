...
import type { AutoResearchConfig, ExperimentRun, Hypothesis, RunMetrics } from "../../../../src/core/research/types.js";
...
describe("ExperimentRunner — Integration", () => {
	...
	it("runs A/B experiment with 2 hypotheses and compares results", async () => {
		const controlHyp: Hypothesis = {
			...
			parameters: { stealthLevel: 0.5 },
		};
		const treatmentHyp: Hypothesis = {
			id: "hyp_treat",
			description: "treatment",
			variant: "treatment_fast",
			changeDescription: "lower stealth",
			parameters: { stealthLevel: 0.2 },
		};
	...
	it("runSingleArm records a single run to journal", async () => {
		...
		const run = await runner.runSingleArm(hyp, { description: "test", maxIterations: 10 }, "test.com", loopFactory);
