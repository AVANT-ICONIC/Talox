...
import type { TaloxPageState, TaloxNode } from '../../src/types/index';
...
describe('AXTreeDiffer', () => {
  ...
  it('detects added nodes', () => {
    ...
    const after = makeState([
      makeNode({ id: 'a' }),
      makeNode({ id: 'b', role: 'link', name: 'New Link' }),
    ]);
  ...
  it('detects removed nodes', () => {
    const before = makeState([
      makeNode({ id: 'a' }),
      makeNode({ id: 'b', role: 'heading', name: 'Gone' }),
    ]);
  ...
  it('detects moved nodes (position change > 30px)', () => {
    const before = makeState([
      makeNode({ id: 'a', boundingBox: { x: 0, y: 0, width: 100, height: 40 } }),
    ]);
    const after = makeState([
      makeNode({ id: 'a', boundingBox: { x: 100, y: 200, width: 100, height: 40 } }),
    ]);
    const result = differ.diff(before, after);
  ...
  it('does not flag small movements (< 30px) as moved', () => {
    ...
    const after = makeState([
      makeNode({ id: 'a', boundingBox: { x: 10, y: 10, width: 100, height: 40 } }),
    ]);
    const result = differ.diff(before, after);
  ...
  it('detects attribute changes', () => {
    const before = makeState([
      makeNode({ id: 'a', attributes: { disabled: false, ariaLabel: 'old' } }),
    ]);
    const after = makeState([
      makeNode({ id: 'a', attributes: { disabled: true, ariaLabel: 'new' } }),
    ]);
    const result = differ.diff(before, after);
    const changed = result.changes.filter(c => c.type === 'changed');
  ...
  it('handles movement direction correctly (right and down)', () => {
    const before = makeState([
      makeNode({ id: 'a', boundingBox: { x: 0, y: 0, width: 50, height: 50 } }),
    ]);
    const after = makeState([
      makeNode({ id: 'a', boundingBox: { x: 200, y: 200, width: 50, height: 50 } }),
    ]);
    const result = differ.diff(before, after);
