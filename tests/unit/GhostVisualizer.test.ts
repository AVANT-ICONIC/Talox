...
import { createGhostVisualizer, GhostVisualizer } from "../../src/core/GhostVisualizer";
...
describe("GhostVisualizer", () => {
	...
	describe("visualize", () => {
		...
		it("returns a buffer for dots style", () => {
			...
			const actions = [makeActionFrame({ visualContext: { mouseX: 10, mouseY: 10 }, relativeTimeMs: 0 })];
	...
	describe("setStyle / setOptions", () => {
		it("setStyle changes the visualization style", () => {
			...
			const actions = [makeActionFrame({ visualContext: { mouseX: 50, mouseY: 50 }, relativeTimeMs: 0 })];
			const result = viz.visualize(actions);
			expect(result).toBeInstanceOf(Buffer);
		});
