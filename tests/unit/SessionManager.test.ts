...
describe('SessionManager', () => {
  ...
  describe('launch', () => {
    ...
    it('records launch artifact', async () => {
      ...
      expect(addActionSpy).toHaveBeenCalledWith('launch', expect.objectContaining({
        profileId: 'test-id',
    ...
    it('passes headless: false when observeOptions.headed is true', async () => {
      ...
      await sm.launch('test-id', 'sandbox', { ...defaultSettings } as any, 'chromium', { headed: true, overlay: false, record: false });
  ...
  describe('openPage', () => {
    it('creates a new page/tab and navigates', async () => {
      ...
      sm.pages = [{ collect: vi.fn().mockResolvedValue({ url: '', title: '', timestamp: '', console: { errors: [] }, network: { failedRequests: [] }, nodes: [], interactiveElements: [], bugs: [] }) } as any];
    ...
    it('records openPage artifact', async () => {
      ...
      expect(addActionSpy).toHaveBeenCalledWith('openPage', expect.objectContaining({
        url: 'https://example.com',
      }));
    });
  });
  ...
  describe('freeze/unfreeze', () => {
    it('freeze stops auto-thinking and records artifact', () => {
      const addActionSpy = vi.spyOn(sm.artifactBuilder, 'addAction');
      sm.freeze();
      expect(addActionSpy).toHaveBeenCalledWith('freeze', expect.objectContaining({
    ...
    it('unfreeze records artifact', () => {
      const addActionSpy = vi.spyOn(sm.artifactBuilder, 'addAction');
      sm.unfreeze();
      expect(addActionSpy).toHaveBeenCalledWith('unfreeze', expect.objectContaining({
        timestamp: expect.any(String),
      }));
    });
  });
  ...
  describe('generateBehavioralDNA', () => {
    ...
    it('records artifact', () => {
      ...
      expect(addActionSpy).toHaveBeenCalledWith('generateBehavioralDNA', expect.objectContaining({
        profileId: 'test',
      }));
    });
  });
  ...
  describe('setHeadedMode', () => {
    it('is no-op when settings.headed already matches', async () => {
      ...
      sm.profile = {
        id: 'test', class: 'sandbox', purpose: 'test', userDataDir: '/tmp/test',
  ...
  describe('performMicroJitter', () => {
    it('moves mouse by small random offset', async () => {
      ...
      expect(setLastMousePos).toHaveBeenCalledWith(expect.objectContaining({
        x: expect.any(Number),
        y: expect.any(Number),
      }));
    });
  ...
  describe('performSmallCursorMovement', () => {
    it('moves mouse by larger offset with clamping', async () => {
      ...
      await sm.performSmallCursorMovement(
        mockPage, { x: 300, y: 400 }, null, clampToFrame, setLastMousePos
      );

      expect(mockPage.mouse.move).toHaveBeenCalledWith(
        expect.any(Number), expect.any(Number), { steps: 2 }
      );
    ...
    it('uses clampToFrame when attention frame is provided', async () => {
      ...
      await sm.performSmallCursorMovement(
        mockPage, { x: 300, y: 400 }, frame, clampToFrame, setLastMousePos
      );
