...
describe('captureSessionSnapshot', () => {
  ...
  it('captures cookies from context', async () => {
    const cookies = [{ name: 'session', value: 'abc123', domain: 'example.com', path: '/', expires: -1, httpOnly: true, secure: false, sameSite: 'Lax' }];
...
describe('restoreSessionSnapshot', () => {
  const baseSnap = {
    ...
    cookies:        [{ name: 'sid', value: 'xyz', domain: 'example.com', path: '/', expires: -1, httpOnly: false, secure: false, sameSite: 'None' as const }],
  ...
  it('does not throw when navigation fails', async () => {
    const context = makeContext();
    const page    = { ...makePage(), goto: vi.fn().mockRejectedValue(new Error('net::ERR_CONNECTION_REFUSED')), evaluate: vi.fn().mockResolvedValue(undefined) };
