...
vi.mock('../../src/core/observe/AnnotationBuffer.js', () => {
  return {
    AnnotationBuffer: class {
      private stack: any[] = [];
      push(entry: any) { this.stack.push(entry); }
...
describe('ObserveSession', () => {
  ...
  describe('constructor', () => {
    ...
    it('respects overlay=false option', () => {
      const page = makeMockPage();
      const context = makeMockContext();
      const session = new ObserveSession(page, context, eventBus, { overlay: false, record: false }, artifactBuilder as any);
      // Creation succeeds
      expect(session.sessionId).toBeTruthy();
    });
  });
  ...
  describe('console error capture', () => {
    it('emits consoleError event when page fires error console message', async () => {
      ...
      expect(errorHandler).toHaveBeenCalledWith(
        expect.objectContaining({ error: 'Uncaught Error: boom' }),
      );
    });
  ...
  describe('network failure capture', () => {
    it('emits networkError event when request fails', async () => {
      ...
      expect(netHandler).toHaveBeenCalledWith(
        expect.objectContaining({ url: 'https://cdn.example.com/script.js' }),
      );
    });
  });
