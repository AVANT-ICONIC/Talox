...
const FONT_PROFILES: Record<FingerprintOS, FontProfile> = {
	windows: {
		systemFonts: [
			...
			"Segoe UI Symbol", "SimSun", "Sylfaen", "Tahoma",
	...
	macos: {
		systemFonts: [
			...
			"Arial Rounded MT Bold", "Avenir", "Avenir Next",
			...
			"Consolas", "Copperplate", "Corbel", "Courier",
			"Courier New", "Didot", "Futura", "Geneva",
			"Georgia", "Gill Sans", "Helvetica", "Helvetica Neue",
			"Herculanum", "Hiragino Sans", "Hoefler Text",
			"Impact", "Menlo", "Monaco", "Noteworthy",
			"Optima", "Palatino", "Papyrus", "PingFang SC",
			"PingFang HK", "PingFang TC", "PT Sans", "PT Serif",
			"San Francisco", "SF Pro Display", "SF Pro Text",
			"Skia", "STHeiti", "STSong", "Sukhumvit Set",
			"Symbol", "Tahoma", "Times", "Times New Roman",
			"Trebuchet MS", "Verdana", "Zapf Dingbats",
			"Zapfino",
		],
	...
	linux: {
		systemFonts: [
			"Arial", "Carlito", "Cantarell", "Caladea", "DejaVu Sans",
			"DejaVu Sans Mono", "DejaVu Serif", "Droid Sans",
			"Droid Sans Mono", "FreeMono", "FreeSans", "FreeSerif",
			"Gayathri", "Gubbi", "Jomolhari", "Kedage",
			"Liberation Mono", "Liberation Sans", "Liberation Serif",
			"Nakula", "Nimbus Mono", "Nimbus Roman", "Nimbus Sans",
			"Noto Color Emoji", "Noto Sans", "Noto Sans CJK SC",
			"Noto Serif", "Paducah", "Sahadeva", "Serif",
			"Standard Symbols PS", "Suruma", "Tibetan Machine Uni",
			"Ubuntu", "Ubuntu Condensed", "Ubuntu Mono", "URW Bookman",
			"URW Chancery", "URW Gothic", "URW Palladio",
		],
		subpixelAA: "rgb",
		letterSpacingOffsetRange: [-0.3, 0.3],
	},
...
export class FingerprintGenerator {
	...
	generate(seed?: string): FingerprintProfile {
		...
		const os = this.weightedPick(
			Object.entries(OS_WEIGHTS) as [FingerprintOS, number][],
			rng,
		);
		...
		// 11. Build profile ID
		const id = this.hashSeed(
			`${uaEntry.ua}:${screen.width}x${screen.height}:${webgl.renderer}:${localeEntry.timezone}`,
		)
			.toString(16)
			.padStart(8, "0");
