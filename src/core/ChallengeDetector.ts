...
const LOGIN_WALL_BODY  = [/sign in to continue/i, /log in to continue/i, /create an account/i, /please log in/i, /login required/i, /sign in required/i];
const LOGIN_WALL_TITLE = [/sign in/i, /log in/i, /login/i];

const CONSENT_BODY  = [/accept.*cookie/i, /cookie.*consent/i, /we use cookies/i, /privacy.*consent/i, /gdpr/i, /accept.*privacy/i];
...
const GEO_BODY  = [/not available in your (country|region)/i, /geo.*restrict/i, /your location/i, /service.*not available.*region/i];
