...
const ACCEPT_SELECTORS = [
  ...
  '[aria-label*="Accept"]', '[aria-label*="accept cookies"]', '[aria-label*="Agree"]',
  // Age-gate confirm
  'button[id*="confirm"]', 'button[id*="age"]', 'button[class*="confirm"]',
  '[data-testid*="accept"]', '[data-testid*="confirm"]',
...
export class ChallengeResolver {
  ...
  async resolve(challenge: DetectedChallenge, page: any): Promise<ChallengeOutcome> {
    switch (challenge.type) {
      case 'cloudflare':    return this.resolveCloudflare(page);
      case 'captcha':       return this.humanHandoff('captcha-present');
      case 'verification':  return this.resolveVerification(page);
      case 'login-wall':    return this.humanHandoff('login-required');
      case 'consent-wall':  return this.resolveConsentWall(page);
      case 'age-gate':      return this.resolveAgeGate(page);
      case 'maintenance':   return this.resolveWithBackoff(page, 'maintenance');
      case 'geo-block':     return this.humanHandoff('policy-blocked');
      case 'rate-limited':  return this.resolveWithBackoff(page, 'rate-limited');
      case 'empty-shell-spa': return this.resolveEmptyShellSpa(page);
      default:              return this.humanHandoff('challenge-unsolved');
    }
  }
  ...
  private async resolveCloudflare(page: any): Promise<ChallengeOutcome> {
    ...
    for (let i = 0; i < this.maxRetries; i++) {
      const delay = Math.min(this.baseDelayMs * Math.pow(2, i), this.maxDelayMs);
      const t0 = Date.now();
      ...
      try {
        ...
        const stillBlocked =
          ...
          /challenge\.cloudflare\.com/i.test(url);

        if (!stillBlocked) {
          attempts.push({ strategy: 'wait-and-settle', success: true, durationMs: Date.now() - t0, detail: `Cleared after ${delay}ms wait` });
          return this.outcome(true, false, attempts, 'wait-and-settle');
        }
      } catch { /* page may still be loading */ }

      attempts.push({ strategy: 'wait-and-settle', success: false, durationMs: Date.now() - t0, detail: `Still blocked (attempt ${i + 1})` });
    }
  ...
  private async resolveConsentWall(page: any): Promise<ChallengeOutcome> {
    ...
    for (const sel of ACCEPT_SELECTORS) {
      try {
        ...
        attempts.push({ strategy: 'auto-click-accept', success: true, durationMs: Date.now() - t0, detail: `Clicked: ${sel}` });
        return this.outcome(true, false, attempts, 'auto-click-accept');
      } catch { /* try next */ }
    }

    attempts.push({ strategy: 'auto-click-accept', success: false, durationMs: Date.now() - t0, detail: 'No accept button found' });
  ...
  private async resolveWithBackoff(page: any, type: 'maintenance' | 'rate-limited'): Promise<ChallengeOutcome> {
    ...
    for (let i = 0; i < this.maxRetries; i++) {
      ...
      try {
        ...
        const stillDown =
          type === 'maintenance'
            ? /maintenance|be right back|service unavailable/i.test(title)
            : false; // for rate-limit we just wait; actual check is on next action

        if (!stillDown) {
          attempts.push({ strategy: 'backoff-retry', success: true, durationMs: Date.now() - t0, detail: `Resolved after reload (attempt ${i + 1})` });
          return this.outcome(true, false, attempts, 'backoff-retry');
        }
      } catch { /* reload failed */ }

      attempts.push({ strategy: 'backoff-retry', success: false, durationMs: Date.now() - t0, detail: `Still blocked (attempt ${i + 1})` });
    }
    ...
    while (Date.now() - t0 < maxWaitMs) {
      ...
      try {
        ...
        const count = await page.evaluate(() =>
          document.querySelectorAll('button, a, input, [role="button"]').length,
        );
        if (count > 2) {
          attempts.push({ strategy: 'wait-hydration', success: true, durationMs: Date.now() - t0, detail: `${count} interactive elements found` });
          return this.outcome(true, false, attempts, 'wait-hydration');
        }
      } catch { /* page still loading */ }
    }

    attempts.push({ strategy: 'wait-hydration', success: false, durationMs: Date.now() - t0, detail: `No interactive elements after ${maxWaitMs}ms` });
    return this.outcome(false, false, attempts, 'wait-hydration');
  }

  private humanHandoff(reason: TakeoverReason): ChallengeOutcome {
    return this.outcome(false, true, [{ strategy: 'human-handoff', success: false, durationMs: 0, detail: `Requires human: ${reason}` }], 'human-handoff', reason);
  }
