...
export class HumanMouse {
	...
	static generatePath(start: Point, end: Point, targetWidth: number = 100, speedMultiplier: number = 1): Point[] {
		...
		for (let i = 0; i <= steps; i++) {
			...
			const easedT = t < 0.5 ? 16 * t * t * t * t * t : 1 - (-2 * t + 2) ** 5 / 2;

			const invT = 1 - easedT;
			let x =
				invT ** 3 * start.x + 3 * invT ** 2 * easedT * cp1.x + 3 * invT * easedT ** 2 * cp2.x + easedT ** 3 * end.x;
			let y =
				invT ** 3 * start.y + 3 * invT ** 2 * easedT * cp1.y + 3 * invT * easedT ** 2 * cp2.y + easedT ** 3 * end.y;
