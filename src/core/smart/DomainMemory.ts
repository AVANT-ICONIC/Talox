...
export class DomainMemory {
  ...
  record(url: string, strategy: string, success: boolean): void {
    ...
    existing.ewmaSuccessRate =
      this.alpha * (success ? 1.0 : 0.0) + (1 - this.alpha) * existing.ewmaSuccessRate;
  ...
  getRankedStrategies(url: string): StrategyScore[] {
    ...
    return Object.values(record.strategies)
      .sort((a, b) => b.ewmaSuccessRate - a.ewmaSuccessRate);
  }
