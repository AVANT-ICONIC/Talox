...
export class ActionExecutor {
	...
	async mouseMove(x: number, y: number): Promise<void> {
		...
		this.artifactBuilder.addAction("mouseMove", {
			x: clampedPos.x,
			y: clampedPos.y,
			hasAttentionFrame: !!attentionFrame,
		});
	...
	async extractTable(selector: string): Promise<Array<Record<string, string>>> {
		...
		return table.evaluate((tableEl: HTMLTableElement) => {
			const headers = Array.from(tableEl.querySelectorAll("thead th, thead td")).map(
				(th) => th.textContent?.trim() || `col${Math.random()}`,
			);
	...
	async findElement(
		...
		lastState?: TaloxPageState | null,
	): Promise<{ selector: string; boundingBox: any } | null> {
		...
		const matches = filtered.filter(
			(e) => e.label.toLowerCase().includes(text.toLowerCase()) || e.name.toLowerCase().includes(text.toLowerCase()),
		);
	...
	private async typeWithTypos(page: any, selector: string, text: string): Promise<void> {
		...
		const dnaTypingDelay = this.getDNATypingDelay(null);
		const baseDelay =
			(dnaTypingDelay.min + Math.random() * (dnaTypingDelay.max - dnaTypingDelay.min)) / settings.mouseSpeed;
	...
	async calculateElementDensity(x: number, y: number): Promise<number> {
		...
		try {
			const density = await page.evaluate(
				async (targetX: number, targetY: number, searchRadius: number) => {
					const allElements = Array.from(
						document.querySelectorAll('button, a, input, select, textarea, [role="button"], [role="link"], [tabindex]'),
					);
