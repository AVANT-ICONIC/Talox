...
export class SessionReporter {
	...
	async write(
		...
		const timestamp = new Date(report.startedAt).toISOString().replace(/[:.]/g, "-").slice(0, 19);
	...
	toMarkdown(report: TaloxSessionReport, extras: SessionReportExtras = {}): string {
		...
		sections.push(...this.renderMarkdownHeader(report));
		sections.push(...this.renderMarkdownSummary(report));
		sections.push(...this.renderMarkdownTimeline(report));
		sections.push(...this.renderMarkdownEventLog(extras.eventLog));
		sections.push(...this.renderMarkdownFailures(extras.failures));
		sections.push(...this.renderMarkdownDiffs(extras.diffs));
		sections.push(...this.renderMarkdownBugs(extras.bugs));
		sections.push(...this.renderMarkdownAnnotationsTable(report.annotations));
		sections.push(...this.renderMarkdownErrorsAndFailures(report.interactions));
		sections.push(...this.renderMarkdownTrace(extras.trace));
	...
	private renderMarkdownSummary(report: TaloxSessionReport): string[] {
		...
		lines.push("| Interactions | Annotations | Console Errors | Network Failures |");
		lines.push("|---|---|---|---|");
		lines.push(
	...
	private renderMarkdownTrace(trace: SessionReportExtras["trace"]): string[] {
		...
		lines.push("| Frame | Type | Action | Detail |");
		lines.push("|---|---|---|---|");
