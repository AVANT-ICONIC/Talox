...
export class SessionReporter {
	...
	async write(
		...
		const safeReport = sanitizeSessionArtifact(report);
		const safeExtras = sanitizeSessionArtifact(mergedExtras);
		...
		await fs.writeFile(jsonPath, JSON.stringify(safeReport, null, 2), "utf-8");
		paths.json = jsonPath;

		if (format === "markdown" || format === "both") {
			const mdPath = path.join(sessionDir, "report.md");
			await fs.writeFile(mdPath, this.toMarkdown(safeReport, safeExtras), "utf-8");
			paths.markdown = mdPath;
		}

		const htmlPath = path.join(sessionDir, "report.html");
		await fs.writeFile(htmlPath, this.toHTML(safeReport, safeExtras), "utf-8");
		paths.html = htmlPath;

		const timelinePath = path.join(sessionDir, "timeline.json");
		await fs.writeFile(timelinePath, JSON.stringify(safeReport.interactions, null, 2), "utf-8");
		paths.timeline = timelinePath;

		const annotationPath = path.join(sessionDir, "annotations.json");
		await fs.writeFile(annotationPath, JSON.stringify(safeReport.annotations, null, 2), "utf-8");
		paths.annotations = annotationPath;

		paths.eventLog = await this.writeJsonIfPresent(sessionDir, "event-log.json", safeExtras.eventLog);
		paths.failures = await this.writeJsonIfPresent(sessionDir, "failures.json", safeExtras.failures);
		paths.diffs = await this.writeJsonIfPresent(sessionDir, "diffs.json", safeExtras.diffs);
		paths.bugs = await this.writeJsonIfPresent(sessionDir, "bugs.json", safeExtras.bugs);
		paths.trace = await this.writeJsonIfPresent(sessionDir, "trace.json", safeExtras.trace);
	...
	toMarkdown(report: TaloxSessionReport, extras: SessionReportExtras = {}): string {
		const safeReport = sanitizeSessionArtifact(report);
		const safeExtras = sanitizeSessionArtifact(extras);
		const sections: string[] = [];
		sections.push(
			...this.renderMarkdownHeader(safeReport),
			...this.renderMarkdownSummary(safeReport),
			...this.renderMarkdownTimeline(safeReport),
			...this.renderMarkdownEventLog(safeExtras.eventLog),
			...this.renderMarkdownFailures(safeExtras.failures),
			...this.renderMarkdownDiffs(safeExtras.diffs),
			...this.renderMarkdownBugs(safeExtras.bugs),
			...this.renderMarkdownAnnotationsTable(safeReport.annotations),
			...this.renderMarkdownErrorsAndFailures(safeReport.interactions),
			...this.renderMarkdownTrace(safeExtras.trace),
	...
	toHTML(report: TaloxSessionReport, extras: SessionReportExtras = {}): string {
		...
		const safeExtras = sanitizeSessionArtifact(extras);
		const summaryRows = `
      <tr><td>Interactions</td><td>${safeReport.summary.totalInteractions}</td></tr>
      <tr><td>Annotations</td><td>${safeReport.summary.totalAnnotations}</td></tr>
      <tr><td>Console Errors</td><td>${safeReport.summary.totalConsoleErrors}</td></tr>
      <tr><td>Network Failures</td><td>${safeReport.summary.totalNetworkFailures}</td></tr>
    `;

		const timelineItems = safeReport.interactions
			.map((i) => `<li>${this.renderInteractionHtml(i, safeReport.annotations)}</li>`)
			.join("");
		...
		const failuresHtml =
			safeExtras.failures
				...
				.join("") ?? "";
		const diffsHtml =
			safeExtras.diffs?.map((diff) => `<li>${this.escapeHtml(diff.url)} changed: ${diff.urlChanged}</li>`).join("") ?? "";
		const bugsHtml =
			safeExtras.bugs
				?.map(
					(bug) =>
				...
				.join("") ?? "";
		const traceFrames = safeExtras.trace ?? [];
      ...
      <html>
        <head>
          <meta charset="utf-8" />
          <title>Talox Session ${safeReport.id}</title>
          <style>
        ...
        <body>
          <header>
            <h1>Talox Session Report · ${this.escapeHtml(safeReport.id)}</h1>
            <p>Started ${this.escapeHtml(safeReport.startedAt)} · Duration ${this.escapeHtml(this.formatDuration(safeReport.durationMs))} · URL ${this.escapeHtml(safeReport.startUrl)}</p>
          </header>
