...
export class InteractionReliability {
  ...
  private async recoverViewport(
    ...
    try {
      ...
      return { resolved: false, mode: 'viewport', attempts, resolvedElement: null, resolvedSelector: selector, recoveryNotes };
    }
  }
  ...
  private async recoverIntercepted(
    ...
    // Step 1: Try pressing Escape (most modals/overlays respond to this)
    {
      ...
      try {
        ...
        if (element && await element.isVisible()) {
          ...
          return { resolved: true, mode: 'intercepted', attempts, resolvedElement: element, resolvedSelector: selector, recoveryNotes };
        }
      } catch { /* Escape did not clear it */ }
    ...
    for (const dismissSel of OVERLAY_DISMISS_SELECTORS) {
      ...
      attempts.push({ mode: 'intercepted', strategy: `dismiss-overlay:${dismissSel}`, success: false, durationMs: Date.now() - t0 });
    }
      ...
      try {
        ...
        if (element) {
          ...
          if (box) {
            ...
            await new Promise(r => setTimeout(r, 200));
            attempts.push({ mode: 'intercepted', strategy: 'dispatch-click', success: true, durationMs: Date.now() - t0 });
    ...
    return { resolved: false, mode: 'intercepted', attempts, resolvedElement: null, resolvedSelector: selector, recoveryNotes };
  }
  ...
  private async recoverDetached(
    ...
    if (bestScore >= 10 && bestNode) {
      ...
      try {
        ...
        return { resolved: true, mode: 'detached', attempts, resolvedElement: element, resolvedSelector: healedSelector, recoveryNotes };
      } catch { /* fall through */ }
    }
    ...
    return { resolved: false, mode: 'detached', attempts, resolvedElement: null, resolvedSelector: selector, recoveryNotes };
  }
  ...
  private async recoverWrongTab(
    ...
    if (!context || typeof context.pages !== 'function') {
      attempts.push({ mode: 'wrong-tab', strategy: 'scan-pages', success: false, durationMs: Date.now() - t0, detail: 'No browser context available' });
      return { resolved: false, mode: 'wrong-tab', attempts, resolvedElement: null, resolvedSelector: selector, recoveryNotes };
    }
    ...
    for (const candidatePage of pages) {
      try {
        ...
        return { resolved: true, mode: 'wrong-tab', attempts, resolvedElement: element, resolvedSelector: selector, recoveryNotes };
      } catch { /* try next page */ }
    }
